...
In this example, the power is about 0.73 for sample size 200.
...
For latent variable models in the mode `"n"`, it is recommended to use
`x_interval` to increase the minimum sample size for the interval
because the default lower limit is 50.
...
[`n_from_power()`](https://sfcheung.github.io/power4mome/reference/x_from_power.md):
    ...
    #> Target Power:                          0.800
    #> 
    #> - Final Value of Sample Size (n): 228
    #> 
    #> - Final Estimated Power (CI): 0.799 [0.781, 0.816]
    #> 
    #> Call `summary()` for detailed results.

In this example, the estimated sample size with power equal to (close
to) the target level (0.80) is 228.

Based on 2000 replications, determined by `final_rep`, the estimated
power for 228 is 0.799, 95% confidence interval \[0.781, 0.816\].
