# Sample Size Determination with Nonnormal Variables
...
with some ready-to-use function for generating random error. A full list
of them can be found
[here](https://sfcheung.github.io/power4mome/articles/reference/index.html#generate-random-numbers).
To be used in
[`power4test()`](https://sfcheung.github.io/power4mome/reference/power4test.md),
these functions need to have the distribution standardized such that the
population mean and standard deviation are 0 and 1, respectively. All
the build-in functions already have the distributions standardized.

For example, the function
[`rexp_rs()`](https://sfcheung.github.io/power4mome/reference/rexp_rs.md)
can be used to approximate a heavily positively skewed distribution. The
function used is
[`stats::rexp()`](https://rdrr.io/r/stats/Exponential.html) from base R,
but, by default, the distribution is transformed such that the
population mean and standard deviation are 0 and 1, respectively.
...
This is part of the output:
    ...
    #> x1    1 60000 0.01 1.01  2.18    20.36  0
    #> x2    2 60000 0.01 1.02  3.33    74.54  0
    #> x3    3 60000 0.01 1.01  2.36    26.60  0
    #> x4    4 60000 0.01 1.02  2.83    46.25  0
    #> m1    5 60000 0.01 1.00  0.86     1.80  0
    #> m2    6 60000 0.00 0.99  0.83     1.74  0
    #> m3    7 60000 0.00 1.00  0.85     1.91  0
    #> y1    8 60000 0.00 1.00 -0.01    -0.28  0
    #> y2    9 60000 0.00 1.00  0.00    -0.28  0
    #> y3   10 60000 0.00 1.00  0.00    -0.29  0
...
`lavaan` is MLR. Although this method and the default, maximum
likelihood (ML), yield the same point estimates, their standard errors
are different. Therefore, the results from Monte Carlo confidence
...
This is part of the output:
    ...
    #>                                               Standard      Scaled
    #>   Test Statistic                                49.815      54.501
    #>   Degrees of freedom                                32          32
    #>   P-value (Chi-square)                           0.023       0.008
...
#> 1 0.096 0.993  0.213  0.182  0.248
#> Notes:
...
also support the `e_fun` and `fit_model_args` arguments. They are set in
the same way as in
[`power4test()`](https://sfcheung.github.io/power4mome/reference/power4test.md)
