...
To simplify the workflows for some
common mediation models, a set of
"quick" functions have been added.
Users can do common tasks using just one
function.

- Added `q_power_mediation()` and
  friends for common mediation models.
  Users can do most steps in one call.
  (0.1.1.18, 0.1.1.19)
...
- Added `merge_all_tests` to
  ...
  `collapse` can then be used for collapsing
...
- Added the `test_method` argument for
  ...
  to do the tests. This change is
  for using the efficient method
  by Boos and Zhang (2000) to estimate
  the rejection rates for resampling
  methods. (0.1.1.3)
...
When calling the function `power4test()`,
the generated data can now be further
processed by *data processors*: functions
to manipulate the generated data, such
as introducing missing values or computing
scale scores. Some built-in processors
are included, but user-supplied functions
can also be used.

- Added `scale_scores()`
  for replacing the indicator scores by
  the corresponding scale scores
  before fitting a model. This allows
  estimating power when the data generating
  model is a latent factor model with
  indicators but the model will be fitted
  to the scale scores instead of the indicators.
  (0.1.1.43)

- Added `missing_values()` for generating
  missing values. This allows sample size
  determination that takes into account
  probable missing data. This also allows
  comparing the levels of power between
  two different methods to handle missing
  data, such as full information maximum
  likelihood versus listwise deletion.
  (0.1.1.66)

- Added `ordinal_variables()` for
  converting continuous indicator variables
  to ordinal variables. This allows
  estimating power when the indicators
  are ordinal variables, such as binary
  items or 5-point Likert scales. This also
  allows comparing the levels of power
...
Support for multigroup models have been
substantially improved. Users can
generate data from a
multigroup models. Some tests can also
be used for estimating power in detecting
group differences, such as the group
...
proposed by Waeber et al. (2013) has
been added. For some scenarios, it allows
for a more efficient search for a
sample size with sufficient power, especially
when a high precision is desired (by
using a large value of `nrep`).
...
- Improved the function for extending the
...
- Skipped the check for combining objects
...
- For probabilistic bisection, the
  ...
  adjusted. This algorithm should be used
  with a wide enough initial interval
  because the interval will not be
  adjusted during the search (for now).
  (0.1.1.56)

- Increased `delta_tol` fo PBA (2 for `n`
  and .002 for `es`).
  (0.1.1.57)

- Added `pba_diagnosis()` to generate
...
- Updated test functions that used
  ...
  zero. Used by the Boos-Zhang-2000
  method. (0.1.1.4)
...
- Fixed duplicated values of x when
  extending the interval. (0.1.1.15)
...
- Added `nls_options` to `power_curve()`
...
- The attribute `number_of_indicators`
  ...
  Used by data processors such as
  `scale_scores()`.
  (0.1.1.43)

- The arguments in `control` are now
...
- The internal helper `do_FUN()` will
  reproduce the search path by loading
  the packages in the workers.
  (0.1.1.69)

- Added and updated technical appendices
...
- Fixed a bug in extending intervals
  ...
  improved the way intervals are extended.
  (0.1.1.35)
