...
# For a path moderated, the coefficent
...
# The test by CI is equivalent to the one-tailed t-test.
...
#> The models:
#> y ~ x + w + x:w
#> <environment: 0x000002062a461c10>
#> 
#> 
#> ====================== Test(s) Conducted ======================
#> 
...
plot(n_power_region)
#> Error in c.power4test_by_n(tmp1, tmp2[i]): Not all objects are based on the same model.
```
...
```
#> Error in c.power4test_by_n(tmp1, tmp2[i]) : 
#>   Not all objects are based on the same model.
if (inherits(tmp, "try-error")) browser()
#> Called from: eval(expr, envir)
#> ℹ Loading power4mome
```
...
#> - Time spent in the search: 21.9 secs 
...
#> - Time spent in the search: 22.92 secs 
#> - The final crude model for the power-predictor relation:
#> 
#> Model Type: Logistic Regression 
#> 
