...
sample size has sufficient power. If
a higher precision is desired, set
`nrep` to 1000 or 2000 for the sample
size to be used.
...
The estimated power is 0.988,
95% confidence interval
[0.971; 0.995], much higher than
...
range of sample sizes, and time is not
a concern in searching for the sample size
with the target level of power.
...
tries to make the results reproducible.
Note that reproducibility is always possible
due to parallel processing. Use a larger
`nrep` to ensure stability of the results
if necessary.
...
The method `plot()` can also be used
to plot the results:
...
<img src="power4test_latent_mediation_plot-1.png" alt="Power by Sample Size"  />
<p class="caption">Power by Sample Size</p>
</div>
