...
By default, `power4mome` uses this convention
...
For a product term in moderation, this is
the convention:
...
- `n`: The sample size in each replications.
  ...
  number can be set to a large value unless
...
By default, the population model
...
power of using Monte Carlo confidence intervals
...
- `ci_type`: The method used to
  ...
  preferable to use Monte Carlo confidence
  intervals to do
...
0.055,
95% confidence interval [0.037; 0.082].
...
This brief illustration only covers the
...
- The population model can be a model
  ...
  indicators. An introduction can be
  found in `vignette("power4test_latent_mediation")`.
  Examples can be found in
  [templates](https://sfcheung.github.io/power4mome/articles/)
