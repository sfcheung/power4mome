...
``` r
gen_missing(data, ..., prop = 0.5, mech = "MCAR")
```
...
`gen_missing()`: `prop` and `mech`. The argument `prop` is defined to
...
[`mice::ampute()`](https://amices.org/mice/reference/ampute.html). For
`gen_missing()`, the default value is `"MCAR"`, missing completely at
random.
